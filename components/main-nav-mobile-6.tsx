...
import { CornerDownRight, List, X } from "lucide-react";
import { Dialog } from "@headlessui/react";
import IconButton from "./ui/icon-button";

interface MainNavMobile6Props {
    data: Category[];
  };

const MainNavMobile6: React.FC<MainNavMobile6Props> = ({
    data
}) => {
    ...
    return ( 
    <>
        <div className="items-center justify-center hidden xl:block space-x-4">
            <Button onClick={onOpen} className="button flex items-center rounded-full bg-black px-4 py-2 space-x-4 h-9">
                <List size={20} />
                <div className="flex items-center ml-1">Kategoriler</div>
            </Button>
        </div>
